...
class Order extends CI_Controller
{
    ...
    public function print_order()
    {
        ...
        $data['order_data'] = $this->Order_model->get_order($order_no);
        $data['shipping'] = $this->Shipping_model->GetCourierOneData($data['order_data'][0]->id_mst_courier);
                    ...
                    if ($row_trn->gateway != 'COD' || $row_trn->gateway != 'cod') {
                        ...
                        foreach ($row_pro as $value_pro) {
                            ...
                            if ($row_ord->coupon_id != 0) {
                                $product_per = number_format((float)(($value_pro->total_price / $row_ord->total_amt) * 100), 2, '.', '');  //44

                                $refund_per = number_format((float)(($product_per / 100) * $row_ord->discount_amt), 2, '.', ''); //22

                                $refund_amt = number_format((float)($value_pro->total_price - $refund_per), 2, '.', '');
                            } else {
            ...
            if ($status == 5) {
                ...
                if ($row_trn->gateway != 'COD' || $row_trn->gateway != 'cod') {
                    ...
                    foreach ($row_pro as $value) {
                        if ($row_ord->coupon_id != 0) {
                            $product_per = number_format((float)(($value->total_price / $row_ord->total_amt) * 100), 2, '.', '');  //44

                            $refund_per = number_format((float)(($product_per / 100) * $row_ord->discount_amt), 2, '.', ''); //22

                            $refund_amt = number_format((float)($value->total_price - $refund_per), 2, '.', '');

                            $new_payable_amt = number_format((float)($row_ord->payable_amt - $refund_amt), 2, '.', '');
                        } else {
